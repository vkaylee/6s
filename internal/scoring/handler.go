// Package scoring provides scoring and leaderboard HTTP handlers.
package scoring
...
func (h *Handler) GetLocationLeaderboard(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		_ = response.AppError(w, r, apperror.Internal(i18n.ErrLeaderboardFailed).WithCause(err))
		return
	}
...
func (h *Handler) UpdateRules(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, ErrMissingReason) {
			_ = response.AppError(w, r, apperror.BadRequest(i18n.ErrMissingRulesReason))
			return
		}
		if errors.Is(err, ErrInvalidRules) {
			_ = response.AppError(w, r, apperror.BadRequest(i18n.ErrInvalidRules))
			return
		}
		_ = response.AppError(w, r, apperror.BadRequest(i18n.ErrBadRequest).WithCause(err))
		return
