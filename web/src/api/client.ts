...
function parseEnvelope(value: unknown): ApiEnvelope<unknown> {
  ...
  if (error !== undefined && (typeof error !== "object" || error === null || typeof (error as Record<string, unknown>).code !== "string" || typeof (error as Record<string, unknown>).message !== "string")) {
    throw new ApiError(502, "Invalid API error response", "INVALID_RESPONSE");
  }
  const pagination = body.pagination;
  if (pagination !== undefined && (typeof pagination !== "object" || pagination === null || typeof (pagination as Record<string, unknown>).page !== "number" || typeof (pagination as Record<string, unknown>).limit !== "number" || typeof (pagination as Record<string, unknown>).total !== "number")) {
    throw new ApiError(502, "Invalid API pagination response", "INVALID_RESPONSE");
  }
  return { data: body.data, error: error as ApiEnvelope<unknown>["error"], pagination: pagination as ApiEnvelope<unknown>["pagination"] };
}

const baseUrl = import.meta.env.VITE_API_BASE_URL || (typeof window !== "undefined" ? window.location.origin : "http://localhost") + "/api";
async function requestFetch(this: unknown, input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  if (input instanceof Request && !init) {
    init = { method: input.method, headers: input.headers, body: input.method === "GET" || input.method === "HEAD" ? undefined : await input.clone().text() };
    input = input.url;
  }
  const requestUrl = typeof input === "string" ? input : input instanceof URL ? input.href : input.url;
  const fetchInput: RequestInfo | URL = typeof window === "undefined" && requestUrl.startsWith("http://localhost") ? new URL(requestUrl).pathname + new URL(requestUrl).search : input;
  ...
  if (!skipAuth && accessToken && !headers.has("Authorization")) headers.set("Authorization", `Bearer ${accessToken}`);
  ...
  if (response.status === 401 && !skipAuth) {
    ...
    if (!newToken) throw new ApiError(401, "Phiên đăng nhập đã hết hạn hoặc đang ngoại tuyến", "UNAUTHORIZED");
...
sdkClient.interceptors.error.use((error, response) => {
  if (response) {
    ...
    throw new ApiError(response.status, typeof apiError?.message === "string" ? apiError.message : `Lỗi máy chủ (${response.status})`, typeof apiError?.code === "string" ? apiError.code : undefined, apiError?.details);
  }
  throw error;
});
...
export async function refreshAccessToken(): Promise<string | null> {
  ...
  isRefreshing = true;
  try {
    const refreshUrl = typeof window === "undefined" ? "/api/auth/refresh" : `${baseUrl}/auth/refresh`;
...
export async function apiClient<T>(url: string, options: RequestOptions = {}): Promise<T> {
  if (options.body && (!options.method || options.method.toUpperCase() === "GET")) throw new TypeError("apiClient mutation requests require an explicit HTTP method");
  const path = url.startsWith("/api") ? url.slice(4) || "/" : url;
  const result = await sdkClient.request({
    url: path,
    method: (options.method ?? "GET") as Uppercase<HttpMethod>,
    body: options.body instanceof FormData ? options.body : typeof options.body === "string" ? JSON.parse(options.body) : options.body,
    headers: { ...Object.fromEntries(new Headers(options.headers).entries()), ...(options.skipAuth ? { "X-Skip-Auth": "true" } : {}) },
    parseAs: "json",
    responseStyle: "data",
    throwOnError: true,
  });
