...
export function CreateIssuePage({ locations, tags, onSuccess }: CreateIssuePageProps) {
  ...
  const handleToggleTag = (tagCode: string) => {
    ...
    if (selectedTags.includes(tagCode)) {
      ...
      // Tag-First auto-categorization: if no category yet or user clicked a tag with clear category
      if (tagObj?.category) {
  ...
  const handleAddCustomTag = () => {
    ...
    // Generate code from query: "Rác rơi vãi" -> "custom_rac_roi_vai"
    ...
    <div className="min-h-screen bg-zinc-100 dark:bg-black text-zinc-900 dark:text-zinc-100 font-sans pb-28">
      ...
      <main className="py-4">
        <PageContainer className="space-y-6">
          {/* 1S - 6S Selection with Micro-hints and Decision Wizard */}
          <section className="bg-white dark:bg-zinc-900 rounded-3xl p-5 border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 min-h-[36px] px-2 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/40"
              >
                <span>💡</span>
          ...
          {/* Full 6S Tag Matrix: Enterprise Premium Instant Search & Taxonomy */}
          {localTags.length > 0 && (
            <section className="bg-white dark:bg-zinc-900 rounded-3xl p-5 border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider">
                ...
                <span className="text-[11px] font-bold text-zinc-400">({localTags.length})</span>
              </div>
              {/* Instant Search Bar */}
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {(() => {
                  ...
                  const visibleTags = localTags.filter((t) => {
                    // When searching, bypass the category tab to search across all tags
                    const matchTab =
                      isSearching || activeTagTab === "ALL" || t.category === activeTagTab;
                    if (!matchTab) return false;
                    if (!isSearching) return true;

                    const labelViNorm = normalizeSearchText(t.label_vi);
                    const labelZh = t.label_zh.toLowerCase();
                    const labelEn = t.label_en ? normalizeSearchText(t.label_en) : "";
                    const tagCode = t.tag_code.toLowerCase();
            ...
            {wizardStep === 1 ? (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 pt-1">
                  {[
                    { key: IssueCategory.S1, text: t("issue.wizard_opt_1s"), badge: "1S" },
                    { key: IssueCategory.S2, text: t("issue.wizard_opt_2s"), badge: "2S" },
                    { key: IssueCategory.S3, text: t("issue.wizard_opt_3s"), badge: "3S" },
                    { key: IssueCategory.S4, text: t("issue.wizard_opt_4s"), badge: "4S" },
                    { key: IssueCategory.S5, text: t("issue.wizard_opt_5s"), badge: "5S" },
                  ].map((opt) => (
                    <button
                      key={opt.key}
                      type="button"
